@props(['book'])

<div class="mt-8 ">
    <a  id="book-show" data-id="{{$book->id}}" href="{{route('book.show',$book['id'])}}">
        @if($book->image)
            <img src="{{asset('storage/'.$book->image)}}" alt="Book Image"
                 class="hover:opacity-75 transition ease-in-out duration-300">
        @else
            <img src="/img/book-cover2.jpg" alt="Book Image"
                 class="hover:opacity-75 transition ease-in-out duration-300">
        @endif
    </a>
    <div class="mt-2">
        <a href="{{route('book.show',$book->id)}}"
           id="book-show" data-id="{{$book->id}}" class="text-lg mt-2 hover:text-blue-600  ">{{$book->title}}</a>
        <div class="flex items-center text-gray-600 text-sm mt-1">
            <svg class="fill-current text-yellow-500 w-4" viewBox="0 0 24 24">
                <g data-name="Layer 2">
                    <path
                        d="M17.56 21a1 1 0 01-.46-.11L12 18.22l-5.1 2.67a1 1 0 01-1.45-1.06l1-5.63-4.12-4a1 1 0 01-.25-1 1 1 0 01.81-.68l5.7-.83 2.51-5.13a1 1 0 011.8 0l2.54 5.12 5.7.83a1 1 0 01.81.68 1 1 0 01-.25 1l-4.12 4 1 5.63a1 1 0 01-.4 1 1 1 0 01-.62.18z"
                        data-name="star"/>
                </g>
            </svg>
            {{--            <span class="ml-1">{{$book['vote_average'] * 10 . '%'}}</span>--}}
            <span class="ml-1">76%</span>
            <span class=" mx-2">|</span>
            {{ \Carbon\Carbon::parse($book->published_at)->format('M d, Y')}}

        </div>
        {{--        category--}}
        <div class="text-gray-400 text-sm">
            {{--            @foreach($book['genre_ids'] as $genre)--}}
            {{--                {{$genres->get($genre)}}@if(!$loop->last), @endif--}}
            {{--            @endforeach--}}
        </div>
    </div>
</div>
